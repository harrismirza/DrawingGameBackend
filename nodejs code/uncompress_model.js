/*
uncompress_model.js
Uncompress JSON model to b64 encoded version to save bandwidth. only works for decoder-only sketch-rnn model.
*/
...
try {
  ...
  console.log("Usage: node uncompress_model.js compressed_model.json orig_full_model.json ")
  process.exit(1);
}
...
var compressed_weights = orig_model[2];
var model_weights = [];

for (var i=0;i<compressed_weights.length;i++) {
  model_weights.push(string_to_array(compressed_weights[i]));
}

var target_model = [orig_model[0], orig_model[1], model_weights];
